...
$email = $payload['email'];
$fullname = isset($payload['name']) ? $payload['name'] : '';
...
try {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        // Asegurar que las columnas Nombre exista (MySQL 8+ soporta IF NOT EXISTS)
        try {
            $db->exec("ALTER TABLE usuarios_alborada ADD COLUMN IF NOT EXISTS Nombre VARCHAR(255) DEFAULT NULL");
        } catch (PDOException $e) {
            // Si ALTER falla por versión de MySQL, ignoramos
        }
        
        // Insertar nuevo usuario con contraseña generada e incluir nombre si existe
        try {
            $ins = $db->prepare('INSERT INTO usuarios_alborada (Correo, Contraseña, Nombre) VALUES (:email, :password, :fullname)');
            $ins->bindParam(':email', $email);
            $ins->bindParam(':password', $hashed_password);
            $ins->bindParam(':fullname', $fullname);
            $ins->execute();
        } catch (PDOException $e) {
            // Si falla con Nombre, intentar sin Nombre
    ...
    $base_url = dirname($_SERVER['REQUEST_URI'], 3) . '/index.php';
    if (substr($base_url, 0, 1) !== '/') {
        $base_url = '/' . $base_url;
    }

    echo json_encode(['success' => true, 'message' => 'Login con Google exitoso.', 'redirect' => $base_url]);
    exit;
