...
// Validar que el pago sea al menos $20
$payment_amount = MINIMUM_PAYMENT_AMOUNT;
if ($payment_amount < MINIMUM_PAYMENT_AMOUNT) {
    http_response_code(400);
    echo json_encode(['success' => false, 'message' => 'El monto mínimo de pago es $' . MINIMUM_PAYMENT_AMOUNT]);
    exit;
}
...
try {
    ...
    $sqlCreatePagos = "CREATE TABLE IF NOT EXISTS pagos (
        id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    $preference = array(
        "items" => array(
            array(
                ...
                "picture_url" => "https://tu-dominio.com/uploads/logo.png", // Cambia esto
                "quantity" => 1,
        ...
        "back_urls" => array(
            "success" => "https://tu-dominio.com/php/payment_success.php",
            "failure" => "https://tu-dominio.com/php/payment_failure.php",
            "pending" => "https://tu-dominio.com/php/payment_pending.php"
        ),
        "auto_return" => "approved",
        "external_reference" => "reserva_" . date('YmdHis') . "_" . rand(1000, 9999),
        "notification_url" => "https://tu-dominio.com/php/webhooks/mercadopago.php",
        "statement_descriptor" => "ALBORADA HOTEL",
        "expires" => true,
        "expiration_date_from" => date('Y-m-d\TH:i:s.000O'),
        "expiration_date_to" => date('Y-m-d\TH:i:s.000O', strtotime('+1 day')),
    ...
    if (!isset($mpResponse['id'])) {
        ...
        echo json_encode([
            'success' => false,
            'message' => 'Error en la respuesta de Mercado Pago',
            'details' => $mpResponse
        ]);
        exit;
    }
